...
- Авторизація через JWT та 3 фіксовані ролі: `client`, `driver`, `dispatcher`
...
## Авторизація (JWT, тільки 3 користувачі)

Публічна реєстрація вимкнена. Вхід доступний тільки для 3 вбудованих користувачів:

- `dispatcher` / `dispatcher123`
- `driver` / `driver123`
- `client` / `client123`

Формат запиту на логін:

- `POST /api/auth/login`
- body: `{ "login": "dispatcher", "password": "dispatcher123" }`
- відповідь: JWT `access_token` (тип `bearer`)

## Тестування

У `backend/tests/` реалізовані базові тести:
- вхід попередньо визначеним користувачем + доступ до профілю
- повний сценарій замовлення з автопризначенням водія
- перевірка, що реєстрація вимкнена
